...
# curl
RUN apt-get update -qq && apt-get install -y -qq libcurl4-openssl-dev php5-curl && apt-get clean
RUN docker-php-ext-install -j$(nproc) curl

# gd
RUN apt-get update -qq && apt-get install -y -qq libpng12-dev libjpeg-dev && apt-get clean
...
# mcrypt
RUN apt-get update -qq && apt-get install -y -qq libmcrypt-dev && apt-get clean
...
# imagick
RUN apt-get update -qq && apt-get install -y -qq libmagickwand-6.q16-dev && apt-get clean
...
# soap
RUN apt-get update -qq && apt-get install -y -qq libxml2-dev && apt-get clean
RUN docker-php-ext-install -j$(nproc) soap

# cron
RUN apt-get update -qq && apt-get install -y -qq cron && apt-get clean
